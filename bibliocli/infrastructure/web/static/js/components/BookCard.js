...
export const BookCard = {
  render(book, onDownloadRaw, onOpenFlow) {
    ...
    card.innerHTML = `
        ...
        <div class="actions">
                    ${book.reader_link 
                        ? `<a href="${book.reader_link}" target="_blank" class="btn-core read-online">
                            READ ONLINE (HTML)
                           </a>`
                    ...
                    <div class="btn-aux open-flow">PROTOCOL: .JSON</div>
                </div>
            </div>
        `;

    // Modern Event Delegation within the component
    const downloadBtn = card.querySelector(".download-raw");
    if (downloadBtn) {
        downloadBtn.addEventListener("click", () => onDownloadRaw(book.link));
    }
