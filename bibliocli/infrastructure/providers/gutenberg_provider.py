...
class GutenbergProvider(BookSearchProvider, BookDownloadProvider):
    ...
    def search(self, query: str) -> List[BookSearchResult]:
        ...
        from bibliocli.application.interfaces import ProviderUnavailableError
        results = []
        try:
            ...
            for item in items[:20]:
                ...
                results.append(BookSearchResult(
                    ...
                    reader_link=item.get("formats", {}).get("text/html") or item.get("formats", {}).get("text/html; charset=utf-8")
                ))
    ...
    def download(self, url: str, destiny_path: str) -> bool:
        ...
        for p_url in possiveis_urls:
            try:
                print(f"Tentando baixar {p_url}...")
                r = requests.get(p_url, headers=headers, timeout=10)
                if r.status_code == 200:
                    content_type = r.headers.get('Content-Type', '').lower()
                    if 'text/plain' not in content_type and 'application/epub+zip' not in content_type and 'application/octet-stream' not in content_type:
                        print(f"Aviso: URL {p_url} retornou 200 mas o tipo é {content_type}, ignorando.")
                        continue

                    if len(r.content) < 1000: # Mínimo 1KB para um livro
                        print(f"Aviso: Arquivo baixado de {p_url} parece muito pequeno ({len(r.content)} bytes).")
                        continue
                    # Checar a extensão do url para nomear o caminho corretamente ou avisar o chamador
                    # Mas o chamador já tem o destiny_path, que é algo como /tmp/bibliocli_abdc.txt.
                    # Vamos salvar como está, pois abrimos em 'wb'
                    with open(destiny_path, "wb") as f: 
                        f.write(r.content)
                    return True
            except Exception as e:
                print(f"Erro ao tentar link {p_url}: {e}")
        ...
        try:
            ...
            if "title" in r:
                ...
                return BookSearchResult(
                    ...
                    reader_link=r.get("formats", {}).get("text/html") or r.get("formats", {}).get("text/html; charset=utf-8")
                )
        except Exception:
            pass
        return None
